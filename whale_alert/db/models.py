...
def init_db() -> None:
    ...
    # Ensure the id column has an auto-incrementing identity
    with engine.connect() as conn:
        result = conn.execute(
            text(
                """
                ...
                WHERE table_name = 'whale_alerts' AND column_name = 'id'
                """
            )
        ).first()
        column_default = result[0] if result else None
        is_identity = result[1] if result else "NO"

        if not column_default and is_identity != "YES":
            conn.execute(
                text(
            ...
            logger.info("Added identity to whale_alerts.id column")
